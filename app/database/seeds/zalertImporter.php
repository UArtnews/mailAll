...
class zalertImporter extends Seeder {
    ...
    public function run()
    {
       ...
       // $zipmailInstance = Instance::where('name', 'Zipmail')->first();
		...
		$zalertInstance = Instance::where('name', 'Z-Alert')->first();

        //$instancesToDelete = Instance::where('name', 'Zipmail')->orWhere('name','Waynemail')->get();
		// $instancesToDelete = Instance::where('name','Waynemail')->get();
		$instancesToDelete = Instance::where('name','zalert')->get();
//Deleting old
...
//        }
//Deprecated This Since the launch of the Zipmail
...
*/
        //Entire ZALERT Process
        $articleMap = array();
        $pubMap = array();

        DB::reconnect('zalert');
        $everything = DB::connection('zalert')
            ->select('Select * from ZALERT
                where ZALERT.ACTIVE = \'0\' and ZALERT.DTTM_CREATED > \'2000-01-01\'');
        ...
        foreach($everything as $current => $thing) {
            //Each Article
            if (!isset($articleMap[$thing->ID])) {
                $newArticle = new Article;

                $newArticle->instance_id = $zalertInstance->id;
                $newArticle->title = $thing->SUBJECT;
                $newArticle->content = $thing->CONTENT;
                $newArticle->author_id = 1;
                $newArticle->published = 'N';
                $newArticle->submission = 'Y';
                if($thing->DTTM_CREATED==null || $thing->DTTM_CREATED==="") {
                	$newArticle->created_at = $_SERVER['REQUEST_TIME'];
				} else {
					$newArticle->created_at = $thing->DTTM_CREATED;
				}
				if($thing->DTTM_LAST_MODIFIED==null || $thing->DTTM_LAST_MODIFIED=="") {
                	$newArticle->updated_at = $_SERVER['REQUEST_TIME'];
				} else {
					$newArticle->updated_at = $thing->DTTM_LAST_MODIFIED;
				}
                $newArticle->issue_dates = json_encode('');

                $newArticle->save();
                $articleMap[$thing->ID] = $newArticle->id;
            }
            //Each publication
            if (!isset($pubMap[$thing->ID])) {
                $newPub = new Publication;

                $newPub->instance_id = $zalertInstance->id;
                $newPub->publish_date = $thing->DTTM_SENT;
                $newPub->banner_image = '';
                $newPub->published = $thing->DTTM_SENT != null ? 'Y' : 'N';
                $newPub->type = 'special';

                $newPub->save();
                $pubMap[$thing->ID] = $newPub->id;
            }
