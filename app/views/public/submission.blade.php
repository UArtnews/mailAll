...
<div class="well col-lg-8 col-lg-offset-2 col-md-10 col-md-offset-1 col-sm-12">
    ...
    <span id='title_error' class="label label-danger" style="display:none;"></span>
        @if(isset($article))
              <input type="text" id="aTitle" class="form-control" onblur="updateTitle()" onkeyup="updateTitle()" value="{{ stripslashes(trim($article->title)) }}"/><br />
        @else
              <input type="text" id="aTitle" class="form-control" onblur="updateTitle()" onkeyup="updateTitle()" placeholder="Announcement Title"/><br />
        @endif
    
    <label for="aContent">Announcement Body:</label><br/>
    <span id='content_error' class="label label-danger" style="display:none;"></span>
    	@if(isset($article))
                    <div id="aContent" class="submissionEditor articleContent form-control" contenteditable="true" onblur="updateContent()" onkeyup="updateContent()" style="padding:5px;background-color:#fff;height:100%;min-height:34px;">{{ stripslashes($article->content) }}</div>
        @else
                    <div id="aContent" class="submissionEditor articleContent form-control" contenteditable="true" onblur="updateContent()" onkeyup="updateContent()" style="padding:5px;background-color:#fff;height:100%;min-height:34px;" data-text="Announcement Body"></div>
        @endif
   </div>
   ...
   <h3>Announcement Preview</h3>
    <div class="well colorPanel">
        <div class="contentDiv">
            ...
            <div class="article" id="article">
                ...
                @if(isset($article))
                    {{ stripslashes(trim($article->title)) }}
                @else
                    Announcement Title
                @endif
                </h1>
    ...
    <div style="margin-left:15px;margin-bottom:10px;"><input type="checkbox" onclick="toggleEvent()" name="eventToggle" id="eventToggle" /> <label for="eventToggle">Include the start and end time of my event, and its location</label><br/></div>
    ...
    <br/>
    <div style="background-color:#eee;padding:5px;border:1px solid #333;border-radius:5px;">
    	<h3>Event Contact Information</h3>
        <div style="margin-left:15px;">
            @if(isset($article))
                @if($article->publish_contact_info=="Y")
                    {{ Form::checkbox('publish_contact_info', '1', true) }}&nbsp;&nbsp;
                @else
                    {{ Form::checkbox('publish_contact_info', '1', false) }}&nbsp;&nbsp;
                @endif
            @else
            {{ Form::checkbox('publish_contact_info', '1', false) }}&nbsp;&nbsp;
            @endif
            ...
            <span id='contactName_error' class="label label-danger" style="display:none;"></span>
            {{ Form::text('contactName', isset($article) ? $article->contactName : null, array('class' => 'form-control')) }}
            <br/>
            {{ Form::label('contactEmail', 'Contact Email: ') }}
            <span id='contactEmail_error' class="label label-danger" style="display:none;"></span>
            {{ Form::text('contactEmail', isset($article) ? $article->contactEmail : null, array('class' => 'form-control')) }}
            <br/>
            {{ Form::label('contactPhone', 'Contact Phone: ') }}
            <span id='contactPhone_error' class="label label-danger" style="display:none;"></span>
            {{ Form::text('contactPhone', isset($article) ? $article->contactPhone : null, array('class' => 'form-control')) }}
            <br/>
        </div>
    </div>
    </div>
    ...
    <h3>Submitted By (will not be published)</h3>
    <div style="margin-left:15px;">
    ...
    {{ Form::close() }}
    @if(isset($article))
    <button id="submitAnnouncement" class="btn btn-success btn-block" onclick="saveSubmission()">Save Edits</button>
    @else
        ...
        $('.submissionEditor').click(function(){
            ...
            CKEDITOR.inline(this, {
                toolbar: [
                     ...
                     { name: 'paragraph', groups: [ 'list', 'blocks', 'align', 'bidi' ], items: [ 'NumberedList', 'BulletedList', '-', 'JustifyLeft', 'JustifyCenter', 'JustifyRight'] },
                     { name: 'others', items: [ '-' ] },
                     { name: 'about', items: [] },
                     '/',
...
<script>
        ...
        headers: {
            ...
            console.log("{{ isset($article) ? 'put' : 'post' }}");
			console.log("ready");
			console.log('{{ isset($article) ? $article : "none" }}');
			var type = "{{ isset($article) ? 'put' : 'post' }}";
			//var type = "post";
    ...
    @if(isset($article))
        @if($article->event_start_date!=null && $article->event_start_date!="" && $article->event_start_date!="1970-01-01")
        <script>
        toggleEvent();
        document.getElementById('eventToggle').checked = true;
        </script>
        @endif
    @endif
</div>
